...
def pendulum(t: Tensor, u: Tensor) -> Tensor:
    ...
    x2 = t[:, 1]
    x1_next = x1 + tau*x2
    x2_next = x2 + tau*((g/l)*Tensor.sin(x1) + (u/(m*l**2)))
...
def mse(output: Tensor, eta: float) -> Tensor:
    se = (output-eta)**2
    return se.mean()
...
if __name__ == "__main__":
    ...
    full_range_hole1, full_range_hole2 = Tensor.arange(-math.pi/6, -math.pi/30, step=epsilon), Tensor.arange(math.pi/30, math.pi/6, step=epsilon)
    ...
    def train_both() -> Tensor:
        ...
        new_barrier = barrier(pendulum(full_range_hole, controller_output))
        loss5 = new_barrier - barrier(full_range_hole)
        loss5 = mse(loss5, eta)
